...
class Reader:
    ...
    def __next__(self):
        ...
        dict['0'] = self.sift(particles, 0) #define your index for particles from hard/bkg/mixing events
        dict['1'] = self.sift(particles, 1)
    ...
    def pseudojet(particle):
        ...
        #for cms forest file, we have the "ParticleFlowAnalyzer" collection, that record (pt, eta, phi, m) of each particle
        particle1.SetPtEtaPhiM(p[0], p[1], p[2], p[3]) #SetPtEtaPhiM(pt, eta, phi, m)->four vectors (Px, Py, Pz, E)
        temp = fj.PseudoJet(particle1.Px(), particle1.Py(),particle1.Pz(), particle1.E()) #Construct PseudoJet object from (Px, Py, Pz, E)
