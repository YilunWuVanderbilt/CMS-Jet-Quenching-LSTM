###########################################
#
# Read EFlow objects from Delphes events
#
###########################################
...
class Reader:
    ...
    def pseudojet(particle):
        ...
        if p[4] == 0 or p[4] == 22:
            pt = sqrt((p[3]**2-p[0]**2)/(tanh(p[1]))**2-(p[3]**2-p[0]**2)) #Calculate pt from ET
            particle1.SetPtEtaPhiE(pt, p[1], p[2], p[3]) #Construct 4-vector object from (pT, eta, phi, E)--see fastjet doc 
